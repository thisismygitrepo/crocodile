...
data = {'category': ['A', 'B', 'A', 'C', None, 'B', 'C', 'A']}
df = pl.DataFrame(data)
encoder = OneHotEncoder(sparse=False, handle_unknown='ignore')  # scikit-learn encoder expects pandas/numpy
fit_data = df.filter(pl.col('category').is_not_null()).to_pandas()
encoder.fit(fit_data[['category']])
transform_data = pl.DataFrame({'category': ['A', 'B', 'D', None, np.nan, None]}).to_pandas()
transformed = encoder.transform(transform_data[['category']])
...
data = {'category': ['A', 'B', 'A', 'C', None, 'B', 'C', 'A']}
df = pl.DataFrame(data)
encoder = OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1, categories=[['C', 'A', 'B']])
fit_data = df.filter(pl.col('category').is_not_null()).to_pandas()
encoder.fit(fit_data[['category']])
transform_data = pl.DataFrame({'category': ['A', 'B', 'D', None, np.nan, None]}).to_pandas()
transformed = encoder.transform(transform_data[['category']])
print("Encoded categories (ordinal):")
print(encoder.get_feature_names_out(['category']))
print(transformed)
