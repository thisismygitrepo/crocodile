"""
Experiment 03: IO Binding + Preallocated Buffers + Adjust Threading

Goals:
* Use IO binding to avoid per-run feed dict overhead.
* Preallocate input/output numpy arrays and reuse.
* Try reduced intra_op threads (e.g., half physical) to mitigate oversubscription for this small model.
* Enable graph optimizations.

Results (CPUExecutionProvider) 2025-09-14:
Threads intra=11 inter=1 phys=22

Batch    PyTorch (ms)   ONNX (ms)    Speedup
1        0.501          0.053        9.41x
32       0.497          0.119        4.19x
128      0.588          0.340        1.73x
512      1.326          1.439        0.92x
1024     3.417          3.567        0.96x

Observations:
* IO binding improved ONNX latency for medium/large batches compared to Experiment 02 (e.g., 512 batch 2.032 -> 1.439 ms; 1024 batch 4.556 -> 3.567 ms) but still not beating PyTorch at largest sizes.
* PyTorch latency increased for tiny batch vs Exp02 (likely due to lower intra threads) but ONNX still maintains strong small-batch speed.
* Model is tiny; large-batch advantage limited because Torch kernels are already efficient and overhead fraction shrinks.
* Next ideas: (a) Compare against torch.compile optimized model; (b) Try sequential executor: so.execution_mode = ORT_SEQUENTIAL for small model; (c) Fuse conv stack manually or widen channels to amplify compute where ORT optimizations can shine.
"""
from __future__ import annotations

import os
import time
import multiprocessing as mp
import numpy as np
import torch
import torch.nn as nn
import onnxruntime as ort
from crocodile.deeplearning_torch import save_all
from crocodile.deeplearning import HParams, Specs, get_hp_save_dir
from crocodile.file_management import P


class PredictionModel(nn.Module):
    def __init__(self, input_channels: int, context_size: int):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels=input_channels, out_channels=8, kernel_size=4, stride=1, padding=2)
        self.conv2 = nn.Conv1d(in_channels=8, out_channels=4, kernel_size=4, stride=1, padding=2)
        self.conv3 = nn.Conv1d(in_channels=4, out_channels=1, kernel_size=4, stride=1, padding=1)
        self.linear = nn.Linear(in_features=59, out_features=20)
        self.linear2 = nn.Linear(in_features=20, out_features=10)
        self.linear3 = nn.Linear(in_features=10, out_features=2)
        self.linear_context1 = nn.Linear(in_features=context_size, out_features=16)
        self.linear_context2 = nn.Linear(in_features=16, out_features=8)
        self.gelu = nn.GELU()
        self.lrelu = nn.LeakyReLU(negative_slope=0.3)

    def forward(self, signals: torch.Tensor, context: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:  # type: ignore[override]
        x = self.gelu(self.conv1(signals))
        x = self.gelu(self.conv2(x))
        x = self.gelu(self.conv3(x))
        x = x.view(x.size(0), -1)
        c = self.gelu(self.linear_context1(context))
        c = self.gelu(self.linear_context2(c))
        x = torch.cat([x, c], dim=1)
        x = self.lrelu(self.linear(x))
        x = self.gelu(self.linear2(x))
        x = self.linear3(x)
        return x, x * 2


def export_model(model: PredictionModel, hp: HParams, specs: Specs) -> str:
    save_all(model=model, hp=hp, specs=specs, history=[{"train": [0.1], "test": [0.1]}])
    return str(get_hp_save_dir(hp=hp).joinpath("model.onnx"))


def make_session(onnx_path: str, intra: int, inter: int) -> ort.InferenceSession:
    so = ort.SessionOptions()
    so.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
    so.enable_mem_pattern = True
    so.enable_cpu_mem_arena = True
    so.intra_op_num_threads = intra
    so.inter_op_num_threads = inter
    return ort.InferenceSession(onnx_path, sess_options=so, providers=["CPUExecutionProvider"])


def measure(model: PredictionModel, session: ort.InferenceSession, sequence_length: int, batch_sizes: list[int], num_runs: int) -> list[dict[str, float]]:
    results: list[dict[str, float]] = []
    io = session.io_binding()
    input_signal_name = session.get_inputs()[0].name
    input_context_name = session.get_inputs()[1].name
    output_names = [o.name for o in session.get_outputs()]
    for batch_size in batch_sizes:
        # Preallocate arrays
        signals_np = np.random.randn(batch_size, 4, sequence_length).astype(np.float32)
        context_np = np.random.randn(batch_size, 8).astype(np.float32)
        model.eval()
        with torch.no_grad():
            t_sig = torch.from_numpy(signals_np)
            t_ctx = torch.from_numpy(context_np)
            for _ in range(5):
                _ = model(t_sig, t_ctx)
            start = time.perf_counter()
            for _ in range(num_runs):
                _ = model(t_sig, t_ctx)
            pytorch_ms = (time.perf_counter() - start) * 1000 / num_runs
        # IO binding warmup
        for _ in range(5):
            io.bind_input(name=input_signal_name, device_type='cpu', device_id=0, element_type=np.float32, shape=signals_np.shape, buffer_ptr=signals_np.ctypes.data)
            io.bind_input(name=input_context_name, device_type='cpu', device_id=0, element_type=np.float32, shape=context_np.shape, buffer_ptr=context_np.ctypes.data)
            # Let ORT allocate outputs
            for out_name in output_names:
                io.bind_output(out_name)
            session.run_with_iobinding(io)
            io.clear_binding_inputs()
            io.clear_binding_outputs()
        start = time.perf_counter()
        for _ in range(num_runs):
            io.bind_input(name=input_signal_name, device_type='cpu', device_id=0, element_type=np.float32, shape=signals_np.shape, buffer_ptr=signals_np.ctypes.data)
            io.bind_input(name=input_context_name, device_type='cpu', device_id=0, element_type=np.float32, shape=context_np.shape, buffer_ptr=context_np.ctypes.data)
            for out_name in output_names:
                io.bind_output(out_name)
            session.run_with_iobinding(io)
            io.clear_binding_inputs()
            io.clear_binding_outputs()
        onnx_ms = (time.perf_counter() - start) * 1000 / num_runs
        results.append({"batch": float(batch_size), "pytorch_ms": pytorch_ms, "onnx_ms": onnx_ms, "speedup": pytorch_ms / onnx_ms})
    return results


def main() -> None:
    torch.manual_seed(42)
    phys = mp.cpu_count()
    intra = max(1, phys // 2)
    inter = 1
    os.environ["OMP_NUM_THREADS"] = str(intra)
    os.environ["MKL_NUM_THREADS"] = str(intra)
    model = PredictionModel(input_channels=4, context_size=8)
    sequence_length = 50
    batch_sizes = [1, 32, 128, 512, 1024]
    hp = HParams(seed=1, shuffle=True, precision="float32", test_split=0.2, learning_rate=0.1, batch_size=32, epochs=1, name="onnx_exp03_io_binding", root=str(P.home().joinpath("tmp_results", "model_root")))
    specs = Specs(ip_shapes={"signals": (4, sequence_length), "context": (8,)}, op_shapes={"output1": (2,), "output2": (2,)})
    onnx_path = export_model(model, hp, specs)
    session = make_session(onnx_path, intra=intra, inter=inter)
    print(f"Providers: {session.get_providers()}")  # noqa: T201
    print(f"Threads intra={intra} inter={inter} phys={phys}")  # noqa: T201
    results = measure(model, session, sequence_length, batch_sizes, num_runs=50)
    header = f"{'Batch':<8} {'PyTorch (ms)':<14} {'ONNX (ms)':<12} {'Speedup':<8}"
    print(header)  # noqa: T201
    print('-' * len(header))  # noqa: T201
    for r in results:
        print(f"{int(r['batch']):<8} {r['pytorch_ms']:<14.3f} {r['onnx_ms']:<12.3f} {r['speedup']:<8.2f}")  # noqa: T201


if __name__ == '__main__':  # pragma: no cover
    main()
