...
class SSH(object):  # if remote is Windows, this class assumed default shell in pwsh, as opposed to cmd
    ...
    def copy_from_here(self, source, target=None, zip_first=False, r=False, overwrite=False):
        ...
        if zip_first: print(f"ZIPPING".center(120, "=")); source = P(source).expanduser().zip()  # .append(f"_{randstr()}", inplace=True)  # eventually, unzip will raise content flag, so this name doesn't matter.
        ...
        print(f"SENDING `{P(source)}` ==> `{remotepath.as_posix()}`".center(120, "=")); self.sftp.put(localpath=P(source).expanduser(), remotepath=remotepath.as_posix()); print(f"all done".center(120, "="), "\n" * 2)
        if zip_first: resp = self.run_py(f"""tb.P(r'{remotepath.as_posix()}').expanduser().unzip(content=True, inplace=True, overwrite={overwrite})""", desc=f"UNZIPPING"); source.delete(sure=True); return resp
    def copy_to_here(self, source, target=None, zip_first=False, r=False):
        ...
        print(f"RECEVING `{source}` ==> `{target}`".center(120, "=")); self.sftp.get(remotepath=source.as_posix(), localpath=target.as_posix()); print(f"all done".center(120, "="), "\n" * 2)
...
def generate_readme(path, obj=None, meta=None, save_source_code=True, verbose=True):  # Generates a readme file to contextualize any binary files by mentioning module, class, method or function used to generate the data"""
    ...
    if save_source_code: P(obj.__code__.co_filename or __import__("inspect").getmodule(obj).__file__).zip(path=readmepath.with_name("source_code.zip"), verbose=False); print("SAVED source code @ " + readmepath.with_name("source_code.zip").absolute().as_uri())
