...
class SSH:  # inferior alternative: https://github.com/fabric/fabric
    ...
    def copy_to_here(self, source, target=None, z=False, r=False) -> P:
        if not z and self.run_py(f"print(tb.P(r'{source}').expanduser().is_dir())", desc="Check if source is a dir", verbose=True, strict_returncode=True, strict_err=True).op.split("\n")[-1] == 'True':
        ...
        if z: source = self.run_py(f"print(tb.P(r'{source}').expanduser().zip(inplace=False, verbose=False))", desc=f"Zipping source file").op2path(strict_returncode=True, strict_err=True)
        ...
        if z: target = target.unzip(inplace=True, content=True); self.run_py(f"tb.P(r'{source}').delete(sure=True)", desc="Cleaning temp files", strict_returncode=True, strict_err=True)
        return target
