...
class Submission:
    ...
    def transfer_sh(machine: RemoteMachine):
        ...
        shell_file = machine.resources.shell_script_path.expanduser()
        shell_script = shell_file.read_text().replace("# EXTRA-PLACEHOLDER-POST", f"cd ~; python {machine.resources.cloud_download_py_script_path.rel2home().as_posix()}")
        ...
        download_url = machine.resources.root_dir.zip().transfer_sh()
        target = machine.resources.root_dir.rel2home().parent.joinpath(download_url.name).as_posix()
        machine.execution_command = f"cd ~; curl -o '{target}' '{download_url.as_url_str()}'; unzip '{target}' -d {machine.resources.root_dir.rel2home().parent.as_posix()}"
        machine.execution_command += f"\n. {machine.resources.shell_script_path.collapseuser().as_posix()}"
...
"""
        ...
        py_script_modified = machine.resources.py_script_path.expanduser().read_text().replace("# EXTRA-PLACEHOLDER-PRE", py_download_script)
        machine.resources.py_script_path.expanduser().write_text(py_script_modified, encoding="utf-8")

        api.upload(machine.resources.root_dir, zip_first=True)
        tb.install_n_import("clipboard").copy((f"bu_gdrive_rx -R {machine.resources.shell_script_path.collapseuser().as_posix()}; " + ("source " if machine.ssh.get_remote_machine() != "Windows" else "")) + f"{machine.resources.shell_script_path.collapseuser().as_posix()}")
